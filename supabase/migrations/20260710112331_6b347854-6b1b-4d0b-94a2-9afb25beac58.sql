...
BEGIN
  ...
  SET exact_score = (p.score_a = m.score_a AND p.score_b = m.score_b),
      good_winner = (
        CASE
      ...
      points = CASE
        ...
        WHEN has_after_regular_time
          AND pr.depot = 'pc_bus'::public.depot
  ...
  PERFORM set_config('app.recomputing_match_points', 'on', true);
  UPDATE public.predictions SET updated_at = now() WHERE match_id = _match_id AND false;
END;
$function$;
...
BEGIN
  ...
  FOR finished_match IN
    SELECT id FROM public.matches WHERE finished = true
  LOOP
    PERFORM public.recompute_match_points(finished_match.id);
  END LOOP;
END $$;