...
BEGIN
  ...
  -- Phase d'ouverture étendue : tout le monde peut choisir jusqu'au 19/06/2026
  IF now() < '2026-06-19T00:00:00Z'::timestamptz THEN
    NEW.updated_at := now();
    RETURN NEW;
  END IF;
...
BEGIN
  ...
  -- Phase d'ouverture étendue : tout le monde peut faire le choix initial jusqu'au 19/06/2026
  IF now() < '2026-06-19T00:00:00Z'::timestamptz THEN
    open_initial := true;
  END IF;
  ...
  IF TG_OP = 'INSERT' OR (TG_OP = 'UPDATE' AND NEW.initial_team_id IS DISTINCT FROM OLD.initial_team_id) THEN
    IF NOT open_initial AND first_kick IS NOT NULL AND now() >= first_kick THEN
