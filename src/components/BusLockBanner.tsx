    ...
    <section className="container mx-auto px-4 py-10">
      ...
      >
        ...
        <div className="relative mx-auto overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-b from-[#05050d] via-[#0a0e1f] to-[#0a0a0f] px-4 pb-2 pt-8 shadow-[0_30px_80px_-20px_rgba(0,0,0,0.8)] sm:px-8 sm:pt-12">
          ...
          {/* Scène d'arrêt : passagers qui attendent puis montent */}
          <div className="pointer-events-none absolute inset-x-0 bottom-1 h-24 overflow-hidden">
            {/* Passagers qui attendent à l'arrêt (côté gauche du poteau) */}
            <div className="absolute bottom-1 left-1/2 flex -translate-x-[140%] items-end gap-1">
              {[0, 1, 2].map((i) => (
                <motion.div
                  key={i}
                  className="relative"
                  animate={{
                    ...
                    x: [0, 0, 30 + i * 8, 30 + i * 8, 30 + i * 8, 0, 0],
                  }}
                  transition={{
                    duration: 14,
                    times: [0, 0.28, 0.42, 0.55, 0.7, 0.85, 1],
                    repeat: Infinity,
                    repeatDelay: 4,
                    ease: "easeInOut",
                    delay: i * 0.15,
                ...
                >
                  {/* tête */}
                  <div className="mx-auto h-1.5 w-1.5 rounded-full bg-zinc-200/90" />
                  {/* corps */}
                  <div
                    className="mt-0.5 h-4 w-2 rounded-t-sm"
                    style={{ background: ["#1e3a8a", "#7f1d1d", "#374151"][i] }}
                  />
                </motion.div>
              ))}
            </div>

            {/* Bus Sequedin : arrive, s'arrête, repart */}
            <motion.div
              className="absolute bottom-0 h-16 sm:h-20"
              initial={{ x: "-50%" }}
              animate={{ x: ["-50%", "30%", "30%", "140%"] }}
              transition={{
                duration: 14,
                times: [0, 0.25, 0.7, 1],
            ...
            >
              <img
                src={sequedinBus.url}
                alt="Bus Sequedin à l'arrêt"
                className="h-full w-auto select-none drop-shadow-[0_8px_12px_rgba(0,0,0,0.6)]"
              />
              {/* Clignotant porte ouverte */}
              <motion.div
                className="absolute -top-1 right-4 h-1.5 w-1.5 rounded-full bg-amber-400"
                animate={{ opacity: [0, 0, 1, 0, 1, 0, 1, 0, 0] }}
                transition={{
                  duration: 14,
                  times: [0, 0.27, 0.32, 0.37, 0.45, 0.52, 0.6, 0.68, 1],
                  repeat: Infinity,
                  repeatDelay: 4,
                }}
